                ...
                for audio_file_addr in tqdm(audio_file_list, desc='Creating features:'):
                    ...
                    if category != 'test':
                        ...
                        def create_windowed_datalist_with_labels(series_data, annotation, win_size, hop_size, feature_file_addr):
                            ...
                            # feature_data[i+1:, :, :] = series_data.loc[first_element_idx_list[-1]: first_element_idx_list[-1] + win_size]
                            # labels[i+1:, :] = annotation.loc[np.ceil(first_element_idx_list[-1]/1764) + 1: np.ceil(first_element_idx_list[-1]/1764) + 24, :]
                            datapoint_num = np.shape(feature_data)[0]
                        ...
                        datapoint_num, feature_data, labels = create_windowed_datalist_with_labels(audio_raw_all, annotation, 44100*0.04*1, 44100*0.04*1, feature_file_addr)
