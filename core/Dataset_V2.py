...
class Dataset(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.validation_total_features = []
        self.validation_total_labels = []
        self.testing_total_features = []
    ...
    def save_features_to_file(self, features, labels, feature_file_addr):
        # features = pd.DataFrame(features)
        # labels = pd.DataFrame(labels)
        # features.to_csv(feature_file_addr, header=False, index=False)
        # labels.to_csv(feature_file_addr + '.label', header=False, index=False)
        np.save(feature_file_addr, features)
        np.save(feature_file_addr+'.labels', labels)
    ...
    def generate_sequencial_batch_data(self, category, num_t_x, overlap=0.5, batch_size=100, input_shape=(1, -1)):
        ...
        generator_idx = 0
        while (1):
            num_of_windows = int(num_data_files / (num_t_x * (1 - overlap)))
            random_perm = np.random.permutation(num_of_windows)
            for i in range(int(np.floor(num_of_windows/batch_size))):
